# Tutorial: [Driver ROS2] driver_read_allstate 노드 구성

이 튜토리얼은 `driver_read_allstate` 예제를 ROS2 노드로 옮겨,
AllState(0xAF) 수신을 토픽으로 publish하는 방법을 다룹니다.

## 목표
1. ROS2 노드에서 Driver 시작
2. `/cmd_vel`을 subscribe해 `setCommand(v, omega)` 업데이트
3. Driver 수신 큐에서 AllState만 처리해 `/allstate_text` publish
...
### AllState 파싱 및 publish
Driver 큐에서 `AllState`만 꺼내 텍스트로 포맷합니다.
...
전체 코드는 `examples/Driver_ROS2/src/driver_read_allstate_node.cpp` 참고.
```cpp
#include "KMC_driver.hpp"
