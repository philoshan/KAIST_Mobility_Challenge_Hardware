# Tutorial: [Driver ROS2] driver_observe 노드 구성

이 튜토리얼은 `driver_observe` 예제를 ROS2 노드로 옮겨,
속도/배터리를 토픽으로 publish하는 구조를 만듭니다.

## 목표
1. ROS2 노드에서 Driver 시작
2. `/cmd_vel`을 subscribe해 `setCommand(v, omega)` 업데이트
3. Driver 수신 큐를 drain 해서 `/vehicle_speed`, `/battery_voltage` publish
...
### Driver 옵션 구성 + 시작
배터리 요청 주기(`battery_hz`)도 옵션에 반영합니다.
...
### 토픽 구성
속도와 배터리 토픽을 publish할 publisher를 만들고,
`/cmd_vel`을 subscribe해 `setCommand()`로 전달합니다.
...
### drain 타이머 + 명령 재전송 타이머
수신 큐는 짧은 주기로 돌리고,
명령은 마지막 값을 주기적으로 업데이트합니다.
...
전체 코드는 `examples/Driver_ROS2/src/driver_observe_node.cpp` 참고.
```cpp
#include "KMC_driver.hpp"
