# Tutorial: [Driver Intermediate] read_allstate (0xAF AllState 읽기)

`Driver`에서 0xAF AllState를 주기적으로 요청하고 큐에서 `AllState`를 처리합니다.

## 목표
- 0xAF AllState 폴링 활성화
- `tryPopMessage()`로 `AllState`를 받아 출력
...
전체 코드는 `examples/Driver_Intermediate/driver_read_allstate.cpp` 참고.
```cpp
