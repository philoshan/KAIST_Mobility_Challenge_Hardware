...
public:
  ...
  {
    for (auto & [topic_bridge, pub_sub_pair] : bridged_topics_) {
      pub_sub_pair.first.reset();
      pub_sub_pair.second.reset();
    }
    bridged_topics_.clear();

    for (const auto & domain_id_node_pair : node_map_) {
      executor.remove_node(domain_id_node_pair.second);
    }
    node_map_.clear();
  }
...
void DomainBridge::remove_from_executor(rclcpp::Executor &executor)
{
  impl_->remove_from_executor(executor);
}
