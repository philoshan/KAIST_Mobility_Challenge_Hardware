  ...
  domain_bridge__msg__CompressedMsg__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    domain_bridge__msg__CompressedMsg * data =
      (domain_bridge__msg__CompressedMsg *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!domain_bridge__msg__CompressedMsg__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          domain_bridge__msg__CompressedMsg__fini(&data[i]);
        }
        free(data);
        return false;
      }
